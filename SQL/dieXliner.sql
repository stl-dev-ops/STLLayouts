-- Query to find liner for a specific die tool number
...
FROM DieJobs dj
OUTER APPLY (
    SELECT TOP 1 mk.art__rpn AS Liner
    ...
    WHERE o2.omschr__ LIKE '%' + ISNULL(dj.MaterialKeyword,'')
      AND (mk.art__rpn LIKE '%LINER%' 
           OR mk.art__rpn LIKE '%LIN%'
           OR mk.art__rpn LIKE '%390%'
           OR mk.art__rpn LIKE '%391%')
      AND mk.art__rpn NOT LIKE '%389%'
    ORDER BY o2.ord__ref DESC
) ln
WHERE ln.Liner IS NOT NULL
