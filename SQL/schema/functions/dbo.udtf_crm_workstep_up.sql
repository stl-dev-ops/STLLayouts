...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[udtf_crm_workstep_up]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
BEGIN
execute dbo.sp_executesql @statement = N'CREATE FUNCTION dbo.udtf_crm_workstep_up (@production_flow_id INT, @workstep_id INT)
RETURNS TABLE 
   AS
RETURN
(
   WITH CTE_recur_Workstep_UP AS
      (
      ...
      SELECT a.production_flow_id,
        ...
        FROM pf_workstep_input__ a
             INNER JOIN pf_workstep_output__  b ON a.production_flow_id = b.production_flow_id
                                               AND a.workstep_id = b.workstep_id
             INNER JOIN CTE_recur_Workstep_UP c ON c.production_flow_id = b.production_flow_id
                                               AND c.input_resource_id = b.resource_id
      )
   SELECT * 
     FROM CTE_recur_Workstep_UP
)' 
END
