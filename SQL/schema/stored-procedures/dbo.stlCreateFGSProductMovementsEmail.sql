...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[stlCreateFGSProductMovementsEmail]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[stlCreateFGSProductMovementsEmail] AS' 
END
-- =============================================
-- Author:		Jeff Rupp
-- Create date: 2/7/2024
-- Description:	Create Email for FGS Product Movments
-- =============================================
ALTER PROCEDURE [dbo].[stlCreateFGSProductMovementsEmail] 
...
BEGIN
	...
	SET @body = '<!DOCTYPE html><html><head><style> table, th, td {padding: 1px 5px 1px 5px;border: 1px solid black;border-collapse: collapse;}</style></head><body>';
		...
		WHERE emails.EmailName = 'FGS Product Movements Report';

		DECLARE fgsCursor CURSOR
		FOR

			SELECT ProductDescription, Customer, [Date], Movement, Quantity, Stock
			FROM vw_stlFGS_ProductMovements 
			ORDER BY ProductID, [Date], OrderOverride, ISNULL([OrderID], ''), ISNULL([SalesOrderID],'')

		OPEN fgsCursor;
		FETCH NEXT FROM fgsCursor INTO
		...
		CLOSE fgsCursor;
		DEALLOCATE fgsCursor;

		INSERT INTO stlEmails ([Subject], Body, RecipientsCSV)
		VALUES ('FGS Product Movements Report', @body, @RecipientsCSV );
	END
END
