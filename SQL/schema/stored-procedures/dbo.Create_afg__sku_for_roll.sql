...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Create_afg__sku_for_roll]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[Create_afg__sku_for_roll] AS' 
END
-- =============================================
-- Author:		<Author,,Name>
...
-- =============================================
ALTER PROCEDURE [dbo].[Create_afg__sku_for_roll]
...
AS
	...
	BEGIN TRANSACTION
		...
		BEGIN TRY
			...
			SET @Ptst__qrt = N'1'
			SET @Pkomm_qrt = N''
			SET @Pinhoud01 = @spool -- N'31'
			SET @Pinhoud02 = @weight --N'6.570'
			SET @Pinhoud03 = @weightPer20K --N'2.830'
			SET @Pinhoud04 = N'20000'
			SET @Psmpl__ex = 0
			SET @Pversiref = N'001'
			SET @Pord2_ref = N''
			SET @Pkom__ref = N''
