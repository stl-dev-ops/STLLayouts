...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_STL_GetMaterialsForPacksize_Dev2]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[SP_STL_GetMaterialsForPacksize_Dev2] AS' 
END
--Exec [SP_STL_GetAllJobsWithCoatingMaterial] '2022-08-15', '2022-09-06', '200003'
ALTER PROCEDURE [dbo].[SP_STL_GetMaterialsForPacksize_Dev2] 
  ...
  BEGIN
	...
	select '************* TEMP Reserved *********************'
	select * from @TempReservedPacking  where art__ref = '100035';
...
--	from @TempReservedPacking t join artikn__ a on t.art__ref = a.art__ref
	...
	select '************* TEMP Reserved after minrewdate *********************'
	select * from @TempReservedPacking  where art__ref = '100035' 
	order by artc_ref, art__ref, nextNeedBy;
...
--		if @DebugLoopCount > 3
	...
	--REAL:select * from @TempOutput order by art__rpn, thisWeekNeeded, nextWeekNeeded


	select '************* TEMP OUT *********************'
	select * from @TempOutput   where art__ref = '100035' 
	order by art__rpn, thisWeekNeeded, nextWeekNeeded 
