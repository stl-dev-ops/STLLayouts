...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[stlQP_QuickPriceUpdate]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[stlQP_QuickPriceUpdate] AS' 
END

-- =============================================
-- Author:		jeff rupp
-- Create date: 07/27/2021
-- Description:	Quick Price Update
-- =============================================
ALTER PROCEDURE [dbo].[stlQP_QuickPriceUpdate]
...
BEGIN TRANSACTION
	...
	UPDATE stlQP_Quotes
	SET kla__ref            = @kla__ref,
		knp__ref		    = @knp__ref,
		vrt__ref			= @vrt__ref,
		ReferenceAtCustomer = @ReferenceAtCustomer,
		QuoteDescription    = @Description,
		ParameterID         = @ParameterID,
		InvoiceStyleID		= @InvoiceStyleID,
		PricingApproved     = @PricingApproved,
		NotReplacingProduct = @NotReplacingProduct,
		prfm_ref            = @prfm_ref,
		Updated             = GETDATE(),
		UpdatedBy           = SUSER_SNAME()
	WHERE ID = @QuoteID;

	UPDATE stlQP_QuoteNotes SET Deleted = 1 WHERE QuoteID = @QuoteID;
	...
	UPDATE stlQP_Label SET Deleted = 1 WHERE QuoteID = @QuoteID;
