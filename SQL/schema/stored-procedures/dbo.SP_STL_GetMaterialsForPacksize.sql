...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_STL_GetMaterialsForPacksize]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[SP_STL_GetMaterialsForPacksize] AS' 
END
--Exec [SP_STL_GetAllJobsWithCoatingMaterial] '2022-08-15', '2022-09-06', '200003'
ALTER PROCEDURE [dbo].[SP_STL_GetMaterialsForPacksize] 
  ...
  BEGIN
	...
	DECLARE @TempOutput TABLE (
	  ...
	  -- 8/5/25: Matthew wants to also show the current in_stock (we're calling onhand)
	insert into @TempOutput 
				select a.art__ref, a.art__rpn, a.art_oms1, (max_stoc - in_stock) + COALESCE( (select sum(reserve_) from resgrd__ r where r.art__ref = a.art__ref group by art__ref), 0), 0, 1, in_stock as onhand
	...
	--Do the reservations
	  ...
	  insert into @TempReservedPacking (art__ref, art__rpn, artc_ref, art_oms1, ord__ref, nextNeedBy, reserve_) 
		select a.art__ref, a.art__rpn, a.artc_ref, a.art_oms1, ord__ref, r.dat_resv, reserve_
										...
										and r.ord__ref not in (select ord__ref from plandv__ where plpl_ref like 'Z_%') 
...
print 'NOW'+ ' @MinRewDate = ' + Convert(varchar(10), @MinRewDate, 101)
		if @MinRewDate IS NOT null
		BEGIN
print 'HEY 18 = ' + ' @MinRewDate = ' + Convert(varchar(10), @MinRewDate, 101)  + @artc_ref + ' tart = ' + @Tart__ref + ' art__Ref = ' + @art__ref + ' ord__ref = ' + @ord__ref + ' nextNeedBy = ' + Convert(varchar(10), @nextNeedBy, 101) + ' Datepart = ' + CONVERT(varchar(4), DATEPART(WEEK, @nextNeedBy)) + ' this week number ' + CONVERT(nvarchar(4), @thisWeekNumber)
			update @TempReservedPacking set nextNeedBy = @MinRewDate where ord__ref = @ord__ref -- and artc_ref = '9'
		END
--		ELSE -- 7-1-24: It's not scheduled yet, so ignore this one somehow, by adding a year to the needbydate
--		BEGIN
--print 'HEY 19 = ' + @artc_ref + ' tart = ' + @Tart__ref + ' art__Ref = ' + @art__ref + ' ord__ref = ' + @ord__ref + ' nextNeedBy = ' + Convert(varchar(10), @nextNeedBy, 101) + ' Datepart = ' + CONVERT(varchar(4), DATEPART(WEEK, @nextNeedBy)) + ' this week number ' + CONVERT(nvarchar(4), @thisWeekNumber)
			...
			--print 'arf'
				...
				BEGIN
					...
					BEGIN
						...
						INSERT INTO @TempOutput (art__ref, art__rpn, art_oms1, thisWeekNeeded, nextWeekNeeded, minmax, onhand) VALUES(@Tart__ref, @Tart__rpn, @Tart_oms1, @TthisWeekNeed, @TnextWeekNeed,0,@onhand)
						SET @blGotSome = 0
					END
				END
...
--		if @DebugLoopCount > 3
	...
	--select * from @TempReservedPacking order by art__ref, nextNeedBy
