...
IF NOT EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tr_crm_hlfvrd___ins_vakverplaatsing]'))
EXEC dbo.sp_executesql @statement = N'CREATE TRIGGER [dbo].[tr_crm_hlfvrd___ins_vakverplaatsing]
   ON  [dbo].[hlfvrd__]
   AFTER INSERT
AS
BEGIN
...
END
' 
ALTER TABLE [dbo].[hlfvrd__] ENABLE TRIGGER [tr_crm_hlfvrd___ins_vakverplaatsing]
