...
IF NOT EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tr_crm_artikd___ins_vakverplaatsing]'))
EXEC dbo.sp_executesql @statement = N'CREATE TRIGGER [dbo].[tr_crm_artikd___ins_vakverplaatsing]
   ON  [dbo].[artikd__]
   AFTER INSERT
AS
BEGIN
   ...
   INSERT INTO dbo.hisvak__ (soort___, ref_____, sku__ref, datum___, uur_____, wuser___ , vak__old, vak__new, jobnr_archive)
      SELECT @soort___, art__ref, artd_ref, @datum___, @uur_____, @wuser___, '''', vak__ref, ''''
      FROM Inserted 
      WHERE vak__ref <> '''' AND vak__ref IS NOT NULL      
END
' 
ALTER TABLE [dbo].[artikd__] ENABLE TRIGGER [tr_crm_artikd___ins_vakverplaatsing]
