...
IF NOT EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tr_crm_hlfvrd___upd_vakverplaatsing]'))
EXEC dbo.sp_executesql @statement = N'CREATE TRIGGER [dbo].[tr_crm_hlfvrd___upd_vakverplaatsing]
   ON  [dbo].[hlfvrd__]
   AFTER UPDATE
AS
BEGIN
   ...
   IF UPDATE (vak__ref)
      BEGIN
         ...
         INSERT INTO dbo.hisvak__ (soort___, ref_____, sku__ref, datum___, uur_____, wuser___ , vak__old, vak__new, jobnr_archive)
            SELECT @soort___, '''', Inserted.hlf__ref, @datum___, @uur_____, @wuser___, Deleted.vak__ref, Inserted.vak__ref, ''''
            FROM Inserted JOIN Deleted on Inserted.hlf__ref = Deleted.hlf__ref
            WHERE Inserted.vak__ref <> Deleted.vak__ref
      END   
END' 
ALTER TABLE [dbo].[hlfvrd__] ENABLE TRIGGER [tr_crm_hlfvrd___upd_vakverplaatsing]
