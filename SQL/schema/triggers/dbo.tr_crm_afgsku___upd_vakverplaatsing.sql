...
IF NOT EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tr_crm_afgsku___upd_vakverplaatsing]'))
EXEC dbo.sp_executesql @statement = N'CREATE TRIGGER [dbo].[tr_crm_afgsku___upd_vakverplaatsing]
   ON  [dbo].[afgsku__]
   AFTER UPDATE
AS
BEGIN
   ...
   IF UPDATE (vak__ref)
      BEGIN
         ...
         INSERT INTO dbo.hisvak__ (soort___, ref_____, sku__ref, datum___, uur_____, wuser___, vak__old, vak__new, jobnr_archive)
            SELECT @soort___, Inserted.afg__ref, Inserted.sku__ref, @datum___, @uur_____, @wuser___, Deleted.vak__ref, Inserted.vak__ref, ''''
            FROM Inserted JOIN Deleted on Inserted.sku__ref = Deleted.sku__ref
            WHERE Inserted.vak__ref <> Deleted.vak__ref
      END   
END' 
ALTER TABLE [dbo].[afgsku__] ENABLE TRIGGER [tr_crm_afgsku___upd_vakverplaatsing]
