...
IF NOT EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[T_spool_ITrig]'))
EXEC dbo.sp_executesql @statement = N'CREATE TRIGGER [dbo].[T_spool_ITrig] ON [dbo].[spool] FOR INSERT AS
SET NOCOUNT ON
...
IF (SELECT COUNT(*) FROM inserted) !=
   (SELECT COUNT(*) FROM spoolJob, inserted WHERE (spoolJob.spoolJobID = inserted.spoolJobID))
    ...
    END' 
ALTER TABLE [dbo].[spool] ENABLE TRIGGER [T_spool_ITrig]
