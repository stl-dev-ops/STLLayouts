...
IF NOT EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tr_crm_artikd___del_vakverplaatsing]'))
EXEC dbo.sp_executesql @statement = N'CREATE TRIGGER [dbo].[tr_crm_artikd___del_vakverplaatsing]
   ON  [dbo].[artikd__]
   AFTER DELETE
AS
BEGIN
...
END
' 
ALTER TABLE [dbo].[artikd__] ENABLE TRIGGER [tr_crm_artikd___del_vakverplaatsing]
