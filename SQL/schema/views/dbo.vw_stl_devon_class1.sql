...
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_stl_devon_class1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.vw_stl_devon_class1
AS
SELECT     dbo.hispnt__.datum___, dbo.hispnt__.wn___rpn, SUM(dbo.GetMinutesFromCERMTimeString(dbo.hispnt__.duur____)) AS Minutes, 
...
GROUP BY dbo.hispnt__.datum___, dbo.hispnt__.wn___rpn, dbo.wpakt___.klasse__
' 
