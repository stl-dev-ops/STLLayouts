...
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_CurrentRewindingClockings]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.vw_CurrentRewindingClockings
AS
...
FROM         (SELECT     ord__ref, SUM(dbo.GetMinutesFromCERMTimeString(stoppen_) - dbo.GetMinutesFromCERMTimeString(starten_)) AS totminutes
                       ...
                       WHERE      (wp___ref IN
                                                  (SELECT     wp___ref
                                                    FROM          dbo.werkpo__
                                                    WHERE      (kenmerk_ = ''Rewinding''))) AND (soort___ = ''2'' OR
                       ...
                       WHERE     (wp___ref IN
                                                 (SELECT     wp___ref
                                                   ...
                                                   WHERE      (kenmerk_ = ''Rewinding'')))
                       GROUP BY ord__ref) AS RewindingingMinutesClocked
GROUP BY ord__ref
ORDER BY ord__ref
' 
