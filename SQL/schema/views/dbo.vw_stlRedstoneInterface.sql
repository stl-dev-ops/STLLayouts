...
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_stlRedstoneInterface]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.vw_stlRedstoneInterface
AS
SELECT     TOP (100) PERCENT b.lbn__ref AS PackingListRef, b.lev__com AS BillingComment, (CASE WHEN b.lok__ref IS NULL OR
                  ...
                  b.lok__ref = '''' THEN CASE WHEN k.land_ref = ''US'' THEN LEFT(k.state___, 2) ELSE k.state___ END ELSE CASE WHEN l.land_ref = ''US'' THEN LEFT(l.state___, 2) ELSE l.state___ END END) AS State, (CASE WHEN b.lok__ref IS NULL OR
                  ...
                  b.lok__ref = '''' THEN konpkl__.knp__vnm + '' '' + konpkl__.knp__nam ELSE c.knp__vnm + '' '' + c.knp__nam END) AS Contact, b.bsbn_kla AS CustPO, lv.omschr__ AS TermsOfDelivery, x.tekst_09 AS Email1, x.tekst_10 AS Email2, x.tekst_11 AS Email3, x.tekst_12 AS Email4, 
                  CASE WHEN x.tekst_16 = '''' OR
                  x.tekst_16 IS NULL THEN CASE b.kla__ref WHEN ''100606'' THEN 1 WHEN ''100525'' THEN 1 WHEN ''100343'' THEN 1 WHEN ''100579'' THEN 1 WHEN ''100611'' THEN 1 WHEN ''100086'' THEN 1 ELSE 0 END ELSE 1 END AS GenericShipment, lb.naam____ AS TransportName
FROM        dbo.bstlyn__ AS b INNER JOIN
                  ...
                  dbo.lev_vw__ AS lv ON b.levvwref = lv.levvwref LEFT OUTER JOIN
                  dbo.levbas__ AS lb ON b.lev__ref = lb.lev__ref
WHERE     (b.lyn__ref IN
