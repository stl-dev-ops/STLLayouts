...
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_stlBA_OrderedDies]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.vw_stlBA_OrderedDies
AS
SELECT     TOP (100) PERCENT ''Ordered'' AS Rectype, ''4418-0008-0000'' AS GLAcct, LEFT(DATENAME(month, bs___dat), 3) AS BudgetMonth, 
