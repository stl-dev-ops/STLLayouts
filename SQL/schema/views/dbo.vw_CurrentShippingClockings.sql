...
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_CurrentShippingClockings]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.vw_CurrentShippingClockings
AS
...
FROM         (SELECT     ord__ref, SUM(dbo.GetMinutesFromCERMTimeString(stoppen_) - dbo.GetMinutesFromCERMTimeString(starten_)) AS totminutes
                       ...
                       WHERE      (wp___ref IN
                                                  (SELECT     wp___ref
                                                    FROM          dbo.werkpo__
                                                    WHERE      (kenmerk_ = ''Shipping''))) AND (soort___ = ''2'' OR
