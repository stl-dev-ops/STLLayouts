...
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_CurrentCohesioClockings]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.vw_CurrentCohesioClockings
AS
...
FROM         (SELECT     ord__ref, SUM(dbo.GetMinutesFromCERMTimeString(stoppen_) - dbo.GetMinutesFromCERMTimeString(starten_)) AS totminutes
                       ...
                       WHERE      (wp___ref IN
                                                  (SELECT     wp___ref
                                                    FROM          dbo.werkpo__
                                                    WHERE      (kenmerk_ = ''Cohesio''))) AND (soort___ = ''2'' OR
                       ...
                       WHERE     (wp___ref IN
                                                 (SELECT     wp___ref
                                                   ...
                                                   WHERE      (kenmerk_ = ''Coating''))) AND (soort___ = ''2'' OR
                                             soort___ = ''b'') AND (stoppen_ <> '' XX:XX'') AND (starten_ <> ''      '') AND (stoppen_ <> ''      '')
                       GROUP BY ord__ref
                       UNION ALL
                       SELECT     ord__ref, SUM(dbo.GetMinutesFromCERMTimeString(stoppen_) - dbo.GetMinutesFromCERMTimeString(starten_)) AS totminutes
                       ...
                       WHERE     (wp___ref IN
                                                 (SELECT     wp___ref
                                                   ...
                                                   WHERE      (kenmerk_ = ''Cohesio'')))
                       GROUP BY ord__ref) AS CoatingMinutesClocked
GROUP BY ord__ref
ORDER BY ord__ref
' 
