...
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_stlScheduledDies_noToestandFilter]'))
...
CREATE VIEW [dbo].[vw_stlScheduledDies_noToestandFilter]
AS
...
WHERE stns_ref <> '''' AND off__ref <> '''' AND sch.StartDate IS NOT NULL --AND sch.toestand <> ''3''
