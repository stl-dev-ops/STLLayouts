...
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_JobProgress_MaxDate]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW dbo.vw_JobProgress_MaxDate
AS
SELECT     dbo.order___.ord__ref,
                          ...
                          (SELECT     SUM(time____) AS Expr1
                            ...
                            WHERE      (ord__ref = dbo.order___.ord__ref)) AS tottime_, dbo.order___.omschr__, dbo.order___.kla__ref, dbo.order___.kla__rpn, dbo.v4eti___.etiket_b, 
                      dbo.v4eti___.etiket_h, dbo.v4eti___.tssnaf_b, dbo.v4eti___.tssnaf_h, dbo.v4eti___.aantal_b, dbo.v4eti___.aantal_h,
                          (SELECT     MAX(datum___) AS Expr1
                            FROM          dbo.vw_PressClockings_per_activity AS vw_PressClockings_per_activity_4
                            WHERE      (ord__ref = dbo.order___.ord__ref)) AS MAXDATUM,
                          (SELECT     MAX(datum___) AS Expr1
                            FROM          dbo.vw_PressClockings_per_activity AS vw_PressClockings_per_activity_4
