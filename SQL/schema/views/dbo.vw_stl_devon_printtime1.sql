...
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_stl_devon_printtime1]'))
EXEC dbo.sp_executesql @statement = N'CREATE VIEW [dbo].[vw_stl_devon_printtime1]
AS
SELECT     dbo.hispnt__.wn___ref, dbo.hispnt__.datum___, dbo.hispnt__.datum___ AS ToDate, dbo.hispnt__.wp___ref, dbo.hispnt__.ord__ref, dbo.hispnt__.wn___rpn, 
                      SUM(dbo.GetMinutesFromCERMTimeString(dbo.hispnt__.duur____)) AS ActMR, dbo.wpakt___.klasse__
...
WHERE     (dbo.hispnt__.wp___ref = ''5001'' OR
                      ...
                      dbo.hispnt__.wp___ref = ''5003'') AND (dbo.wpakt___.klasse__ = ''1'')
