-- Trace jobs consuming extracted materials back to actual facesheet via stlCC SKU linkage
...
ConsumedJobs AS (
    ...
    INNER JOIN ExtractedMaterials em ON m.art__rpn LIKE '%' + em.ExtractedMaterial + '%'
    WHERE ord.ord__ref NOT IN ('', 'N')
      AND c.artd_ref <> ''
),
-- Link consumed SKUs to the jobs that created them via stlCC
SKUCreationJobs AS (
    SELECT DISTINCT
        cj.JobID,
        cj.ConsumedSKU,
        cards.ord__ref AS MaterialCreationJobID
    FROM ConsumedJobs cj
    INNER JOIN stlCC_CoatingCardRolls rolls ON cj.ConsumedSKU = rolls.SKU
    INNER JOIN stlCC_CoatingCards cards ON rolls.CoatingCardID = cards.ID
),
...
SELECT DISTINCT
    ...
    ofa.ActualFacesheetID,
    ofa.ActualFacesheetKeyword,
    ofa.ActualFacesheetDescription,
